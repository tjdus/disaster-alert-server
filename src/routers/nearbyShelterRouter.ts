...
import { FloodShelter, IFloodShelter } from "../schemas/floodShelter";
...
export const nearbyShelterRouter = (...args: any[]) => {
  ...
  return router
    ...
    .get("/flood-shelters", async (req, res) => {
      try {
        const { lat, lon, distance = MAX_DISTANCE_KM } = req.query;
        ...
        // Find shelters within the specified radius
        const shelters = await FloodShelter.aggregate([
          {
            $geoNear: {
              near: {
                type: "Point",
        ...
        const response: ListResponse<IFloodShelter> = {
          data: shelters,
        };
